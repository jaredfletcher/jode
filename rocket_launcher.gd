...
## only what firing actually depends on. It never registers itself with the
## player, never assumes where it sits in the tree, and never touches Input.
## That keeps it swappable, testable on its own, and means the player does not
## have to know what kind of weapon it is holding.
##
...
## launches it.
##
## Two separate corrections happen here and it is worth keeping them apart. One
## is about where the rocket starts, the other about which way it goes.
func _fire(shooter: Player, aim: Basis, eye: Vector3) -> void:
	...
	# eye, so fired straight forward it travels on a line parallel to the view
	# and lands beside what was aimed at, by the width of the offset. Aiming it
	# at the hit point instead makes the two converge, which is why TF2 rockets
	# go where the crosshair is.
	var far := eye + forward * aim_range_units * U
	var target := far
	var ahead := space.intersect_ray(
		PhysicsRayQueryParameters3D.create(eye, far, 0xFFFFFFFF, exclude))
	...
	# The muzzle sits most of two feet in front of the eye, which is far enough
	# to be on the far side of a wall you are standing against. Source traces
	# out to it and starts at the first thing in the way, so a rocket fired into
	# a wall explodes on your side of it.
	var blocked := space.intersect_ray(
		PhysicsRayQueryParameters3D.create(eye, muzzle, 0xFFFFFFFF, exclude))
